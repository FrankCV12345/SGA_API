...
public class SolicitudesPorAlumnoController {
	...
	@PutMapping("{id}/Solicitudes/{idSolicitud}")
	public ResponseEntity<Solcitud_alumno> CancelaSolicitud( @PathVariable("idSolicitud") int idUser ,@RequestBody Solcitud_alumno updatedSolicitud ){
		Solcitud_alumno updated = solicitudAlumnoService.modificarEstado(idUser, updatedSolicitud);
		return new  ResponseEntity<Solcitud_alumno>(updated,HttpStatus.OK);
	}
}
